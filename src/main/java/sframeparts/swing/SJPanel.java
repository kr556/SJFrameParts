...
public class SJPanel extends JPanel implements MouseListener, MouseMotionListener {
    ...
    public SJPanel setSJSize(int w, int h) {
        setSize(w,h);
        return this;
    }
    public SJPanel setSJSize(Dimension d) {
        setSize(d);
        return this;
    }
    public SJPanel setSJBounds(int x, int y, int w, int h) {
        setBounds(x,y,w,h);
        return this;
    }
    public SJPanel setSJBounds(Rectangle r) {
        setBounds(r);
        return this;
    }
    public SJPanel setSJLayout(BorderLayout b) {
        setLayout(b);
        return this;
    }
    public SJPanel setSJBackground(Color c) {
        setBackground(c);
        return this;
    }
    public SJPanel setSJVisible(boolean b) {
        setVisible(b);
        return this;
    }
    public SJPanel setSJBorder(Border b) {
        setBorder(b);
        return this;
    }
    public SJPanel setSJOpaque(boolean b) {
        setOpaque(b);
        return this;
    }
    public SJPanel setSJFont(Font f) {
        setFont(f);
        return this;
    }

    public SJPanel addSJ(Component com) {
        add(com);
        return this;
    }
    /**
     ...
     */
    public SJPanel addKeepColor(Color c) {
     ...
     * @param c         Anything color.
     */
    public SJPanel addKeepColor(String colorName, Color c) {
        if (colorName.equals("")) {
            colorNameID.add(String.valueOf(keepColor.size()));
            keepColor.put(String.valueOf(keepColor.size()), c);
        } else {
    ...
    public class Event {
        ...
        public Event mouse(MouseEventWrapper m) {
            ...
            if (m.getButton() == SMouseEvent.BUTTON_ALL) {
                for (int i = 0; i <= SMouseEvent.BUTTON3; i++) {
                    eventWrappers[m.getWhen() - SMouseEvent.MOUSE_FIRST][i] = new MouseEventWrapper(m.getWhen(),m.getAction(),i);
                }
            } else {
